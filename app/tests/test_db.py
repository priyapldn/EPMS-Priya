...
class TestDatabaseInitialization(unittest.TestCase):
    ...
    @patch('app.database.create_engine')  # Mock SQLAlchemy's create_engine
    @patch('app.database.Base.metadata.create_all')  # Mock the metadata create_all method
    def test_init_engine(self, mock_create_all, mock_create_engine):
        """Test that the engine is initialized and tables are created."""
        
        # Call the init_engine function with a test database URI
        test_db_uri = 'sqlite:///:memory:'
        init_engine(test_db_uri)
        
        # Assert that create_engine was called with the correct database URI
        ...
        # You can also check that engine was set globally
        from app.database import engine
        self.assertEqual(engine, mock_create_engine.return_value)

    @patch('app.database.Session')  # Mock the Session object
    def test_get_session_success(self, mock_session):
        ...
        mock_engine = MagicMock()  # Create a mock engine
        with patch('app.database.engine', new=mock_engine):
            # Call get_session, it should return a session
            session_instance = get_session()

            # Assert that a session was created and bound to the mock engine
    ...
    def test_get_session_no_engine(self):
        ...
        with patch('app.database.engine', new=None):
            # Assert that RuntimeError is raised when engine is not initialized
    ...
    def test_add_employee(self, mock_session):
        ...
        # Mock the count to return 0 (meaning the table is empty)
        session.query.return_value.count.return_value = 0
        
        # Call the function to add an employee
        add_employee(session, "John Doe", 12345, "johndoe1234", "john@example.com", "hashed_password", is_admin=False)
        
        # Check that the new employee was added and session committed
        self.assertEqual(session.add.call_count, 1)  # One add call
        self.assertEqual(session.commit.call_count, 1)  # One commit call
        
        # Verify the correct employee was added
        new_employee = session.add.call_args[0][0]  # Get the employee from the add call
        self.assertIsInstance(new_employee, Employee)  # Should be an instance of Employee
    ...
    def test_add_review(self, mock_session):
        ...
        add_review(session, 12345, "20-09-2024", 1, "Good", "Achieved all goals", "Great performance!")
        
        # Check that the new review was added and session committed
        self.assertEqual(session.add.call_count, 1)  # One add call
        self.assertEqual(session.commit.call_count, 1)  # One commit call
        
        # Verify the correct review was added
        new_review = session.add.call_args[0][0]  # Get the review from the add call
        self.assertIsInstance(new_review, Review)  # Should be an instance of Review
